...
$("#ardoraQuest").html("<img id='playSound' class='imaLeft' src='hangman2_resources/media/' alt='Sound' align='left'>"+ "<p>" + questionW[indexG] + "</p>");
...
function calcMaxWidth(){for (i = 0; i < wordsG.length; i++) {var actualSize=0;actualSize=$.trim(aforWords(wordsG[i])).length*(26+9);
...
showButtonNext();}}}}
function paintLetters(){profG=profG+64; var leV=0;var toV=profG;for (i = 0; i < fillLetter.length; i++) {
...
leV=leV+41;if (leV+41>actMaxWidth){leV=0;toV=toV+41;}}
var heiAct=(toV+41).toString()+"px";$("#ardoraAct").css("height",heiAct);
...
function paintbGame(){profG=profG+14+9;var leV=(actMaxWidth-($.trim(aforWords(wordsG[indexG])).length-1)*(14+19))/2;
...
contexto.fillRect(0, 0, canvas.width, canvas.height);contexto.font = "17px " + fMenssage;
var metricsW = contexto.measureText(textBNext).width;var x = canvas.width - metricsW -50 ;var y = canvas.height-17-40;
...
roundedRect(contexto, x, y, metricsW+45,17+20, 5, colorButton);contexto.shadowColor = "black";contexto.shadowBlur = 15;
...
contexto.fillText(textBNext,x+10,y+17+10);contexto.lineWidth=5;contexto.stroke();contexto.fill();
